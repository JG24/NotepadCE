...
void EditTimeDate()
{
    ...
    wchar_t buf[256];
    const wchar_t *fmt = g_state.dateTimeFormat.empty() ? L"%H:%M %d.%m.%Y" : g_state.dateTimeFormat.c_str();
    if (wcsftime(buf, sizeof(buf) / sizeof(wchar_t), fmt, &tmLocal) == 0)
        wcsftime(buf, sizeof(buf) / sizeof(wchar_t), L"%H:%M %d.%m.%Y", &tmLocal);
